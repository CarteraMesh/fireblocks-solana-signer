...
use {
    base64::prelude::*,
    fireblocks_solana_signer::{FireblocksSigner, VersionedTransactionExtension},
...
pub const LOOKUP: Pubkey = solana_sdk::pubkey!("24DJ3Um2ekF2isQVMZcNHusmzLMMUS1oHQXhpPkVX7WV");
#[allow(dead_code)]
pub const USDC: Pubkey = solana_sdk::pubkey!("4zMMC9srt5Ri5X14GAgXhaHii3GnPAEERYPJgZJDncDU");
pub const TO: Pubkey = solana_sdk::pubkey!("E4SfgGV2v9GLYsEkCQhrrnFbBcYmAiUZZbJ7swKGzZHJ");
...
// let kp = solana_sdk::signature::Keypair::from_base58_string(&kp_secret);
// let hash = rpc.get_latest_blockhash()?;
// let ins = build_memo(&MEMO_ID, "multi".as_bytes(), &[
// &fireblocks_signer.pubkey(),
// &kp.pubkey(),
// ]);
// let message = Message::new_with_blockhash(&[ins],
// Some(&fireblocks_signer.pubkey()), &hash); let mut tx: Transaction =
// Transaction::new_unsigned(message); kp.try_sign_multi_legacy(&mut tx, &[],
// hash)?; // don't need to really do this, i'm just testing if Keypair can see
// this function. fireblocks_signer.try_sign_multi_legacy(&mut tx,
// &[&fireblocks_signer, &kp], hash)?; assert!(tx.is_signed());
// broadcast(&rpc, &tx)?;
// Ok(())
// }
...
//         &fireblocks_signer.pubkey(),
//     ]);
//     let message = Message::new_with_blockhash(&[ins], Some(&kp.pubkey()),
// &hash);     let mut tx: Transaction = Transaction::new_unsigned(message);
//     kp.try_sign_multi_legacy(&mut tx, &[], hash)?;
//     assert!(!tx.is_signed());
//     fireblocks_signer.try_sign_multi_legacy(&mut tx, &[&kp,
// &fireblocks_signer], hash)?;     assert!(tx.is_signed());
...
// BASE64_STANDARD.encode(bincode::serialize(&tx)?);     tracing::info!("
// broadcasting transaction non fireblocks fee payer {transaction_base64}");
...
//     ]);
//     let mut tx =
//         VersionedTransaction::new_unsigned_v0(&fireblocks_signer.pubkey(),
// &[ins], &[], hash)?;     kp.try_sign_multi_versioned(&mut tx,
// &[&fireblocks_signer], Some(hash))?;     assert_ne!(tx.signatures[1],
// Signature::default());     assert_eq!(tx.signatures[0],
// Signature::default());     fireblocks_signer.try_sign_multi_versioned(&mut
// tx, &[&kp], Some(hash))?;     broadcast(&rpc, &tx)?;
//     Ok(())
// }
...
fn test_signer_legacy() -> anyhow::Result<()> {
    ...
    let hash = rpc.get_latest_blockhash()?;
    let message = Message::new(&[memo("test_signer_legacy", &signer.pk)], Some(&signer.pk));
...
fn test_keypair() -> anyhow::Result<()> {
    ...
    let hash = rpc.get_latest_blockhash()?;
    let message = Message::new(&[memo("fireblocks keypair", &signer.pk)], Some(&signer.pk));
    let mut tx = Transaction::new_unsigned(message);
    tx.try_sign(&[&signer], hash)?;
